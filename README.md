...
- **自動実行**: GitHub Actions により、監視時間帯の直前（JST 7:15, 17:15）にジョブが起動します。
  - 一度起動すると、ワークフロー内で **10分おきに計10回（約90分間）** スクリプトが実行されます。
  - これにより、GitHub 側のスケジューラの遅延に左右されず、正確な間隔での監視を実現しています。
- **手動実行**: GitHub の「Actions」タブから「Run workflow」ボタンを押すことで、いつでも即時実行できます。
  - 手動実行時も、自動実行と同様にその時点から **90分間のループ実行** が開始されます。
  - 途中で停止したい場合は、GitHub のジョブ画面から「Cancel run」を押してください。
